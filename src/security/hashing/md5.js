...
module.exports = (input, salt) => {
    ...
    const words = Hash.stringToWords(input);
    ...
    const hashedResult = Hash.toHexLittleEndian(a) + Hash.toHexLittleEndian(b) +
        Hash.toHexLittleEndian(c) + Hash.toHexLittleEndian(d);
    const randomIndex = crypto.randomInt(Math.max(1, crypto.randomInt(100)) * (hashedResult.length + 1));
