...
export default (input: string, salt?: string | null): string => {
    ...
    const binaryInput =
        Hash.stringToBinary(input + String.fromCharCode(0x80)) +
    ...
    for (let i = 0; i < binaryInput.length / 512; i++) {
        ...
        for (let j = 16; j < 64; j++) {
            const s0 =
                Hash.rightRotate(words[j - 15], 7) ^
                Hash.rightRotate(words[j - 15], 18) ^
                (words[j - 15] >>> 3);
            const s1 =
                Hash.rightRotate(words[j - 2], 17) ^
        ...
        for (let j = 0; j < 64; j++) {
            const S1 =
                Hash.rightRotate(e, 6) ^
                Hash.rightRotate(e, 11) ^
                Hash.rightRotate(e, 25);
            const ch = (e & f) ^ (~e & g);
            const temp1 = (h + S1 + ch + K[j] + words[j]) >>> 0;
            const S0 =
                Hash.rightRotate(a, 2) ^ Hash.rightRotate(a, 13) ^ Hash.rightRotate(a, 22);
